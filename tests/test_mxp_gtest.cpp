...
TEST_F(MXPTest, CollectedElementRoutesOpeningTags)
{
    ...
    EXPECT_EQ(doc->m_mxpEngine->m_strMXPstring, "bold");
}

// Test 2: MXP_collected_element ignores comments
TEST_F(MXPTest, CollectedElementIgnoresComments)
{
    doc->m_mxpEngine->m_strMXPstring = "!-- This is a comment --";
...
TEST_F(MXPTest, CollectedElementRoutesClosingTags)
{
    ...
    EXPECT_TRUE(doc->m_mxpEngine->m_strMXPstring.startsWith('/'));
}

// Test 4: MXP_collected_element routes definitions
TEST_F(MXPTest, CollectedElementRoutesDefinitions)
{
    doc->m_mxpEngine->m_strMXPstring = "!ELEMENT hp '<color &col;><send>'";
...
{
    AtomicElement* bold = doc->m_mxpEngine->MXP_FindAtomicElement("bold");
    ...
    AtomicElement* underline = doc->m_mxpEngine->MXP_FindAtomicElement("underline");
...
{
    AtomicElement* lower = doc->m_mxpEngine->MXP_FindAtomicElement("bold");
    AtomicElement* upper = doc->m_mxpEngine->MXP_FindAtomicElement("BOLD");
    AtomicElement* mixed = doc->m_mxpEngine->MXP_FindAtomicElement("Bold");
...
{
    AtomicElement* unknown = doc->m_mxpEngine->MXP_FindAtomicElement("notarealtag");
...
{
    AtomicElement* sound = doc->m_mxpEngine->MXP_FindAtomicElement("sound");
...
{
    AtomicElement* image = doc->m_mxpEngine->MXP_FindAtomicElement("image");
...
{
    AtomicElement* gauge = doc->m_mxpEngine->MXP_FindAtomicElement("gauge");
...
{
    QString lt = doc->m_mxpEngine->MXP_GetEntity("lt");
    EXPECT_EQ(lt, "<");

    QString gt = doc->m_mxpEngine->MXP_GetEntity("gt");
    EXPECT_EQ(gt, ">");

    QString amp = doc->m_mxpEngine->MXP_GetEntity("amp");
    EXPECT_EQ(amp, "&");

    QString quot = doc->m_mxpEngine->MXP_GetEntity("quot");
    EXPECT_EQ(quot, "\"");
}
...
{
    QString nbsp = doc->m_mxpEngine->MXP_GetEntity("nbsp");
    EXPECT_EQ(nbsp, QString(QChar(0xA0)));

    QString copy = doc->m_mxpEngine->MXP_GetEntity("copy");
    EXPECT_EQ(copy, QString(QChar(0xA9)));

    QString reg = doc->m_mxpEngine->MXP_GetEntity("reg");
...
{
    QString unknown = doc->m_mxpEngine->MXP_GetEntity("notarealentity");
...
TEST_F(MXPTest, GetEntityRejectsControlCharacters)
{
    ...
    QString tab = doc->m_mxpEngine->MXP_GetEntity("#9");
    EXPECT_EQ(tab, "\t");

    QString lf = doc->m_mxpEngine->MXP_GetEntity("#10");
    EXPECT_EQ(lf, "\n");

    QString cr = doc->m_mxpEngine->MXP_GetEntity("#13");
    EXPECT_EQ(cr, "\r");

    // Other control characters should be rejected
    QString null_char = doc->m_mxpEngine->MXP_GetEntity("#0");
    EXPECT_TRUE(null_char.isEmpty());

    QString bell = doc->m_mxpEngine->MXP_GetEntity("#7");
...
{
    QString invalid1 = doc->m_mxpEngine->MXP_GetEntity("#");
    EXPECT_TRUE(invalid1.isEmpty());

    QString invalid2 = doc->m_mxpEngine->MXP_GetEntity("#x");
...
{
    QString lower = doc->m_mxpEngine->MXP_GetEntity("nbsp");
    QString upper = doc->m_mxpEngine->MXP_GetEntity("NBSP");
...
TEST_F(MXPTest, ParseMXPTagHandlesSimpleNames)
{
    ...
    doc->m_mxpEngine->ParseMXPTag("bold", tagName, args);
...
TEST_F(MXPTest, ParseMXPTagHandlesSingleArgument)
{
    ...
    doc->m_mxpEngine->ParseMXPTag("color red", tagName, args);
...
TEST_F(MXPTest, ParseMXPTagHandlesNameValueFormat)
{
    ...
    doc->m_mxpEngine->ParseMXPTag("send href='go north'", tagName, args);
...
TEST_F(MXPTest, ParseMXPTagHandlesDoubleQuotes)
{
    ...
    doc->m_mxpEngine->ParseMXPTag("send href=\"go south\" hint=\"Click me\"", tagName, args);
...
TEST_F(MXPTest, ParseMXPTagHandlesMixedQuotes)
{
    ...
    doc->m_mxpEngine->ParseMXPTag("send href='north' hint=\"tooltip\"", tagName, args);
...
TEST_F(MXPTest, ParseMXPTagHandlesMultiplePositionalArgs)
{
    ...
    doc->m_mxpEngine->ParseMXPTag("color red blue", tagName, args);
...
TEST_F(MXPTest, ParseMXPTagHandlesEmptyTag)
{
    ...
    doc->m_mxpEngine->ParseMXPTag("", tagName, args);
...
{
    int initialCount = doc->m_mxpEngine->m_atomicElementMap.size();

    // Call MXP_On again
    doc->MXP_On();

    // Should not duplicate elements
    EXPECT_EQ(doc->m_mxpEngine->m_atomicElementMap.size(), initialCount);
}
...
TEST_F(MXPTest, GetMXPArgumentHelperWorks)
{
    ...
    QString href = doc->m_mxpEngine->GetMXPArgument(args, "href");
    QString hint = doc->m_mxpEngine->GetMXPArgument(args, "hint");
    QString missing = doc->m_mxpEngine->GetMXPArgument(args, "nonexistent");
...
TEST_F(MXPTest, DefineElementParsesSimpleDefinition)
{
    ...
    doc->m_mxpEngine->MXP_DefineElement("hp '<color red><b>'");

    // Check custom element was created
    CustomElement* elem = doc->m_mxpEngine->MXP_FindCustomElement("hp");
    ASSERT_NE(elem, nullptr);
...
{
    doc->m_mxpEngine->m_iMXP_mode = 1;

    // Define element with ATT: <!ELEMENT hp '<color &col;>' ATT='col=red'>
    doc->m_mxpEngine->MXP_DefineElement("hp '<color &col;>' ATT='col=red'");
...
TEST_F(MXPTest, DefineElementParsesTAGKeyword)
{
    ...
    doc->m_mxpEngine->MXP_DefineElement("room '' TAG=25");

    CustomElement* elem = doc->m_mxpEngine->MXP_FindCustomElement("room");
    ASSERT_NE(elem, nullptr);
...
TEST_F(MXPTest, DefineElementParsesOPENKeyword)
{
    ...
    doc->m_mxpEngine->MXP_DefineElement("custom '<send>' OPEN");

    CustomElement* elem = doc->m_mxpEngine->MXP_FindCustomElement("custom");
    ASSERT_NE(elem, nullptr);
...
TEST_F(MXPTest, DefineElementParsesEMPTYKeyword)
{
    ...
    doc->m_mxpEngine->MXP_DefineElement("custom_br '' EMPTY");

    CustomElement* elem = doc->m_mxpEngine->MXP_FindCustomElement("custom_br");
    ASSERT_NE(elem, nullptr);
...
TEST_F(MXPTest, DefineElementCannotRedefineBuiltin)
{
    ...
    doc->m_mxpEngine->MXP_DefineElement("bold '<italic>'");
    ...
    CustomElement* customBold = doc->m_mxpEngine->MXP_FindCustomElement("bold");
...
TEST_F(MXPTest, DefineElementReplacesExisting)
{
    ...
    doc->m_mxpEngine->MXP_DefineElement("test '<b>'");
    doc->m_mxpEngine->MXP_DefineElement("test '<i>'");

    // Should have only one element (the second one)
    CustomElement* elem = doc->m_mxpEngine->MXP_FindCustomElement("test");
    ASSERT_NE(elem, nullptr);
...
TEST_F(MXPTest, DefineEntityCreatesCustomEntity)
{
    ...
    doc->m_mxpEngine->MXP_DefineEntity("hp '100'");

    // Check it resolves correctly
    QString result = doc->m_mxpEngine->MXP_GetEntity("hp");
...
TEST_F(MXPTest, DefineEntityExpandsEmbeddedEntities)
{
    ...
    doc->m_mxpEngine->MXP_DefineEntity("test '&lt;bold&gt;'");

    // Should expand to "<bold>"
    QString result = doc->m_mxpEngine->MXP_GetEntity("test");
...
TEST_F(MXPTest, DefineEntityDELETEKeywordRemovesEntity)
{
    ...
    doc->m_mxpEngine->MXP_DefineEntity("temp '123'");
    EXPECT_EQ(doc->m_mxpEngine->MXP_GetEntity("temp"), "123");

    doc->m_mxpEngine->MXP_DefineEntity("temp DELETE");
    EXPECT_TRUE(doc->m_mxpEngine->MXP_GetEntity("temp").isEmpty());
}
...
TEST_F(MXPTest, CustomElementBOpenFlagEnforcesOpenMode)
{
    ...
    doc->m_mxpEngine->MXP_DefineElement("danger '<send>' OPEN");

    CustomElement* elem = doc->m_mxpEngine->MXP_FindCustomElement("danger");
    ASSERT_NE(elem, nullptr);
    EXPECT_TRUE(elem->bOpen);
...
TEST_F(MXPTest, ActiveTagStackPushesOnOpeningTag)
{
    ...
    EXPECT_GE(doc->m_mxpEngine->m_activeTagList.size(), initialSize);
}
...
TEST_F(MXPTest, EndTagPopsFromActiveTagStack)
{
    ...
    int sizeAfterOpen = doc->m_mxpEngine->m_activeTagList.size();
    ...
    EXPECT_LE(doc->m_mxpEngine->m_activeTagList.size(), sizeAfterOpen);
}
...
TEST_F(MXPTest, TagNoResetPersistsThroughModeChange)
{
    ...
    doc->m_mxpEngine->m_strMXPstring = "version";
    doc->m_mxpEngine->MXP_collected_element();
...
TEST_F(MXPTest, ModeSwitchingTriggersTagCleanup)
{
    ...
    int tagCountInSecure = doc->m_mxpEngine->m_activeTagList.size();
    ...
    EXPECT_LE(doc->m_mxpEngine->m_activeTagList.size(), tagCountInSecure);
}
...
TEST_F(MXPTest, ArgumentExtractionFromTag)
{
    ...
    doc->m_mxpEngine->m_strMXPstring = "send href='north'";
    doc->m_mxpEngine->MXP_collected_element();
...
TEST_F(MXPTest, ColorParsingHexFormat)
{
    ...
    doc->m_mxpEngine->m_strMXPstring = "color fore=#FF0000";
    doc->m_mxpEngine->MXP_collected_element();
...
TEST_F(MXPTest, ColorParsingNamedColor)
{
    ...
    doc->m_mxpEngine->m_strMXPstring = "color red";
    doc->m_mxpEngine->MXP_collected_element();
...
TEST_F(MXPTest, BooleanArgumentDetection)
{
    ...
    doc->m_mxpEngine->m_strMXPstring = "send prompt";
    doc->m_mxpEngine->MXP_collected_element();
...
TEST_F(MXPTest, StateVariableChangesNoBr)
{
    ...
    bool initialNoBr = doc->m_mxpEngine->m_bMXP_nobr;

    // Use nobr tag
    doc->m_mxpEngine->m_strMXPstring = "nobr";
    doc->m_mxpEngine->MXP_collected_element();

    // m_bMXP_nobr should be set to true
    EXPECT_TRUE(doc->m_mxpEngine->m_bMXP_nobr);
    ...
    EXPECT_EQ(doc->m_mxpEngine->m_bMXP_nobr, initialNoBr);
}
...
TEST_F(MXPTest, ColorActionChangesForegroundColor)
{
    ...
    // Use color tag
    doc->m_mxpEngine->m_strMXPstring = "color fore=red";
    doc->m_mxpEngine->MXP_collected_element();
...
TEST_F(MXPTest, HighTagIncreasesColorValues)
{
    ...
    doc->m_mxpEngine->m_strMXPstring = "high";
    doc->m_mxpEngine->MXP_collected_element();
