...
|:---|---:|:---|:---|
| `world/lua_api/world_output.cpp` | 1767 | world | **51 tests added** — moved to Well-Covered |
| `world/world_serialization.cpp` | 1427 | world | **20 tests added** — moved to Well-Covered |
| `world/lua_api/lua_registration.cpp` | 1429 | world | **19 tests added** — moved to Well-Covered |
...
| `world/world_protocol.cpp` | 880 | world | **49 tests added** — moved to Well-Covered |
| `network/ansi_formatter.cpp` | 249 | network | **39 tests added** — moved to Well-Covered |

## Requires GUI Testing Framework

UI modules that cannot be meaningfully unit-tested with GTest alone. These require
a GUI testing tool (Squish, Qt Test input simulation, or screenshot diffing) for
rendering correctness, hit-testing, mouse interaction, and layout verification.

Some files contain testable logic (marked with `*`) that could be extracted and
unit-tested — `savePluginXml` returns `std::expected`, `loadFromFile`/`saveToFile`
return `std::expected`, and DoCommand dispatch has a pure-logic layer.

| Source File | Lines | Module | Testable Logic | Needs GUI Framework For |
|:---|---:|:---|:---|:---|
| `ui/main_window.cpp` | 5461 | ui | — | Menu handling, world lifecycle, window management |
| `ui/views/output_view.cpp` | 2057 | ui | — | Rendering, hit-testing, mouse selection, URL clicks |
| `ui/dialogs/global_preferences_dialog.cpp` | 1540 | ui | Property marshalling `*` | Dialog layout, widget interaction |
| `ui/dialogs/plugin_wizard.cpp` | 1206 | ui | `savePluginXml` `*` | Wizard page flow, UI validation |
| `ui/views/output_view_miniwindows.cpp` | 1039 | ui | — | Miniwindow overlay compositing |
| `ui/views/world_widget.cpp` | 656 | ui | `loadFromFile`/`saveToFile` `*` | Widget lifecycle |
| `ui/main_window_commands.cpp` | 291 | ui | DoCommand dispatch `*` | Commands that require active window |
| `ui/lua_dialog_registration.cpp` | 224 | ui | — | Lua dialog bridge registration |
