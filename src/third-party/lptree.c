...
static TTree* newcharset(lua_State* L, byte* cs)
{
    ...
    switch (op) {
        case IFail:
            return newleaf(L, TFalse); /* empty set */
        case IAny:
            return newleaf(L, TAny); /* full set */
...
static int lp_divcapture(lua_State* L)
{
    switch (lua_type(L, 2)) {
        ...
        default:
            return luaL_error(L, "unexpected %s as 2nd operand to LPeg '/'", luaL_typename(L, 2));
    }
}
...
*/
static int verifyrule(lua_State* L, TTree* tree, unsigned short* passed, int npassed, int nb)
{
tailcall:
    switch (tree->tag) {
        case TChar:
...
static struct luaL_Reg metareg[] = {{"__mul", lp_seq},        {"__add", lp_choice},
                                    {"__pow", lp_star},       {"__gc", lp_gc},
                                    {"__len", lp_and},        {"__div", lp_divcapture},
                                    {"__mod", lp_acccapture}, {"__unm", lp_not},
                                    {"__sub", lp_sub},        {NULL, NULL}};


/*
** lpeg.version() - Returns the LPeg version string
** Made a function for compatibility with code that calls lpeg.version()
*/
...
int luaopen_lpeg(lua_State* L)
{
    ...
    /* Register version as a function for compatibility with code that calls lpeg.version() */
    lua_pushcfunction(L, lp_version);
