 ...
 * Use with GetInfo() to understand what each info type returns.
 *
 * @returns Table where key is info type number, value is description
 *
 * @example
 * for k, v in pairs(utils.infotypes()) do
 *     print(k, v, GetInfo(k))
 * end
 *
 * @see GetInfo
...
{
    // GetInfo type mappings (from original MUSHclient methods_info.cpp)
    static const struct {
        int type;
        const char* desc;
    } infoTypes[] = {
        // Strings - configuration (1-50)
        {1, "Server name"},
        {2, "World name"},
        {3, "Character name"},
        ...
        // Strings - calculated at runtime (51-89)
        ...
        // Booleans - calculated at runtime (101-125)
        {101, "No Echo"},
        ...
        // Numbers (longs) - calculated at runtime (201-300)
        ...
        {231, "Log file size"},
        {232, "High-performance timer"},
        {233, "Trigger execution time"},
        {234, "Alias execution time"},
        ...
        // Dates - calculated at runtime (301-306)
        {301, "Time connected"},
